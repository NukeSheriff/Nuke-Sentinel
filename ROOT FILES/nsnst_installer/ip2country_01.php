...
@set_time_limit(600);
$pagetitle = "IP to Country: Update Step 1";
include("header.php");
title("$pagetitle");
OpenTable();
echo 'Operation Status!<br />'."\n";
echo '<hr />'."\n";
echo 'Goto <a href="'.$_SERVER['PHP_SELF'].'?op=ip2country_02">IP to Country: Update Step 2</a> (Automatically starts in 5 seconds)<br />(If it does not start click the link above.)'."\n";
echo '<hr />'."\n";
$db->sql_query("DELETE FROM `".$prefix."_nsnst_ip2country`");

$wfilename = "nsnst_installer/data/ip2country_1.data";
...
echo '{'."\n";
echo ' location.href = \''.$_SERVER['PHP_SELF'].'?op=ip2country_02\';'."\n";
echo '}'."\n";
echo '// --></script>'."\n";
CloseTable();
include("footer.php");
