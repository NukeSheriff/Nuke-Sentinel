...
$result = $db->sql_query("CREATE TABLE `".$prefix."_nsnst_admins` (`aid` varchar(25) NOT NULL default '', `login` varchar(25) NOT NULL default '', `password` varchar(40) NOT NULL default '', `password_md5` varchar(60) NOT NULL default '', `password_crypt` varchar(60) NOT NULL default '', `protected` tinyint(2) NOT NULL default '0', PRIMARY KEY (`aid`))");
...
if(file_exists($wfilename)) {
  ...
  while (!feof($wfread)) {
    ...
    if($data[0] > "") {
      ...
      $datainserted = False;
      $datainserted = $db->sql_query("INSERT INTO `".$prefix."_nsnst_blockers` VALUES (".$data[0].", '".$data[1]."', ".$data[2].", ".$data[3].", ".$data[4].", '', '".$data[6]."', '".$data[7]."', ".$data[8].", ".$data[9].", '')");
...
$result = $db->sql_query("CREATE TABLE `".$prefix."_nsnst_cidrs` (`cidr` int(2) NOT NULL default '0', `hosts` int(10) NOT NULL default '0', `mask` varchar(15) NOT NULL default '', PRIMARY KEY (`cidr`))");
if(!$result) { echo $mess1; } else { echo $mess2; }
// INSERT INTO TABLE nsnst_cidrs
echo 'Populate '.$prefix.'_nsnst_cidrs: ';
$mess = $mess2;
$wfilename = "nsnst_installer/data/cidrs.data";
if(file_exists($wfilename)) {
  $wfread = fopen($wfilename, 'r');
  $linecount = 0;
  while (!feof($wfread)) {
    ...
    if($data[0] > "") {
      ...
      $datainserted = False;
      $datainserted = $db->sql_query("INSERT INTO `".$prefix."_nsnst_cidrs` VALUES (".$data[0].", ".$data[1].", '".$data[2]."')");
      if(!$datainserted) { $mess = $mess1; }
    }
  }
  fclose($wfread);
...
$result = $db->sql_query("CREATE TABLE `".$prefix."_nsnst_config` (`config_name` varchar(255) NOT NULL default '', `config_value` longtext NOT NULL, PRIMARY KEY (`config_name`))");
if(!$result) { echo $mess1; } else { echo $mess2; }
// INSERT INTO TABLE nsnst_config
echo 'Populate '.$prefix.'_nsnst_config: ';
$mess = $mess2;
$wfilename = "nsnst_installer/data/config.data";
if(file_exists($wfilename)) {
  $wfread = fopen($wfilename, 'r');
  $linecount = 0;
  while (!feof($wfread)) {
    ...
    if($data[0] > "") {
      ...
      $datainserted = False;
      $datainserted = $db->sql_query("INSERT INTO `".$prefix."_nsnst_config` VALUES ('".$data[0]."', '".$data[1]."')");
      if(!$datainserted) { $mess = $mess1; }
    }
  }
  fclose($wfread);
...
$result = $db->sql_query("CREATE TABLE `".$prefix."_nsnst_countries` (`c2c` char(2) NOT NULL default '', `country` varchar(60) NOT NULL default '', PRIMARY KEY `c2c` (`c2c`))");
if(!$result) { echo $mess1; } else { echo $mess2; }
// INSERT INTO TABLE nsnst_countries
echo 'Populate '.$prefix.'_nsnst_countries: ';
$mess = $mess2;
$wfilename = "nsnst_installer/data/countries.data";
if(file_exists($wfilename)) {
  $wfread = fopen($wfilename, 'r');
  $linecount = 0;
  while (!feof($wfread)) {
    ...
    if($data[0] > "") {
      ...
      $datainserted = False;
      $datainserted = $db->sql_query("INSERT INTO `".$prefix."_nsnst_countries` VALUES ('".$data[0]."', '".$data[1]."')");
      if(!$datainserted) { $mess = $mess1; }
    }
  }
  fclose($wfread);
...
$result = $db->sql_query("CREATE TABLE `".$prefix."_nsnst_excluded_ranges` (`ip_lo` int(10) unsigned NOT NULL default '0', `ip_hi` int(10) unsigned NOT NULL default '0', `date` int(20) NOT NULL default '0', `notes` text NOT NULL, `c2c` char(2) NOT NULL default '00', PRIMARY KEY (`ip_lo`,`ip_hi`), KEY `c2c` (`c2c`), KEY `date` (`date`))");
if(!$result) { echo $mess1; } else { echo $mess2; }
...
$result = $db->sql_query("INSERT INTO `".$prefix."_nsnst_excluded_ranges` VALUES ('$admin_ip', '$admin_ip', '".$nsnst_const['ban_time']."', 'Admin\'s personal ip', '00')");
if(!$result) { echo $mess1; } else { echo $mess2; }
...
$mess = $mess2;
$wfilename = "nsnst_installer/data/harvesters.data";
$list_harvester = "";
if(file_exists($wfilename)) {
  $wfread = fopen($wfilename, 'r');
  $linecount = 0;
  while (!feof($wfread)) {
    ...
    if($data[0] > "") {
      ...
      $datainserted = False;
      $datainserted = $db->sql_query("INSERT INTO `".$prefix."_nsnst_harvesters` VALUES (".$data[0].", '".$data[1]."')");
      if(!$datainserted) { $mess = $mess1; }
    }
  }
  fclose($wfread);
...
$result = $db->sql_query("CREATE TABLE `".$prefix."_nsnst_ip2country` (`ip_lo` int(10) unsigned NOT NULL default '0', `ip_hi` int(10) unsigned NOT NULL default '0', `date` int(20) NOT NULL default '0', `c2c` char(2) NOT NULL default '', PRIMARY KEY (`ip_lo`,`ip_hi`), KEY `c2c` (`c2c`), KEY `date` (`date`))");
if(!$result) { echo $mess1; } else { echo $mess2; }
// CREATE TABLE nsnst_protected_ranges
echo 'Create '.$prefix.'_nsnst_protected_ranges: ';
$result = $db->sql_query("CREATE TABLE `".$prefix."_nsnst_protected_ranges` (`ip_lo` int(10) unsigned NOT NULL default '0', `ip_hi` int(10) unsigned NOT NULL default '0', `date` int(20) NOT NULL default '0', `notes` text NOT NULL, `c2c` char(2) NOT NULL default '00', PRIMARY KEY (`ip_lo`,`ip_hi`), KEY `c2c` (`c2c`), KEY `date` (`date`))");
if(!$result) { echo $mess1; } else { echo $mess2; }
// Add current admin ip to protected table
echo 'Populate '.$prefix.'_nsnst_protected_ranges: ';
$admin_ip = sprintf("%u", ip2long($nsnst_const['remote_ip']));
$result = $db->sql_query("INSERT INTO `".$prefix."_nsnst_protected_ranges` VALUES ('$admin_ip', '$admin_ip', '".$nsnst_const['ban_time']."', 'Admin\'s personal ip', '00')");
if(!$result) { echo $mess1; } else { echo $mess2; }
...
$mess = $mess2;
$wfilename = "nsnst_installer/data/referers.data";
$list_referer = "";
if(file_exists($wfilename)) {
  $wfread = fopen($wfilename, 'r');
  $linecount = 0;
  while (!feof($wfread)) {
    ...
    if($data[0] > "") {
      ...
      $datainserted = False;
      $datainserted = $db->sql_query("INSERT INTO `".$prefix."_nsnst_referers` VALUES (".$data[0].", '".$data[1]."')");
      if(!$datainserted) { $mess = $mess1; }
    }
  }
  fclose($wfread);
...
echo '<a href="'.$_SERVER['PHP_SELF'].'?op=ip2country_01">Install IP 2 Country Data</a> (Automatically starts in 5 seconds)<br />(If it does not start click the link above.)'."\n";
echo '<script type="text/javascript"><!--'."\n";
echo 'setTimeout(\'Redirect()\',5000);'."\n";
echo 'function Redirect()'."\n";
echo '{'."\n";
echo ' location.href = \''.$_SERVER['PHP_SELF'].'?op=ip2country_01\';'."\n";
echo '}'."\n";
echo '// --></script>'."\n";
CloseTable();
include("footer.php");
